...
def add():
    ...
    if info_list[1] in RESULT:
        print("用户%s已经存在，请重新输入", info_list[1])
    else:
        # 添加用户，并给予用户提示信息
        RESULT.append(info_list[1:])
        print(">> \033[32m添加用户 {} 成功\033[0m".format(info_list[1]))
    print(info_list[1], info_list)


def delete():
    if user in RESULT:
        # 删除用户，并给予用户提示信息
        print("\033[32m删除用户 {}\033[0m".format(user))
        RESULT.remove(user)
    else:
        print("\033[32m用户 {} 不存在\033[0m".format(user))

def update():
    # 判断用户是否存在, 如果用户存在才允许修改，否则提示告知用户不存在
    if info[1] not in RESULT:
        print("用户info[1]已经存在，请重新输入")
    else:
        # 添加用户，并给予用户提示信息
        RESULT.append(info_list[1:])
        print(">> \033[32m添加用户 {} 成功\033[0m".format(info_list[1]))
