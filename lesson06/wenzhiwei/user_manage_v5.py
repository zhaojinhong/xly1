...
class UserManage(object):
    ...
    def __init__(self):
        self.dbcontrol = DBControl('config.ini', 'dbconfig')

    def user_add(self, *args) -> str:
        user_dict = {}
        ...
        rows, status = self.dbcontrol.dbselect('''select * from users''')
        if status:
            for i in rows:
            ...
            if user_username in user_dict:
                ...
                msg, status = self.dbcontrol.dbinsert(
                    """insert into users(username,age,tel,email) values('%s', '%s', '%s', '%s');""" % (
                        user_username, user_age, user_tel, user_email))
                result = msg
                return result

    def user_delete(self, *args) -> str:
        user_username = args[0]
        operater = args[1]
        msg, status = self.dbcontrol.dbdelete("""delete from users where username='%s';""" % user_username)
        result = msg
        user_log(" ".join([operater, msg, user_username]))
        return result

    def user_update(self, *args) -> str:
        user_username = args[0]
        user_parament = args[1]
        user_value = args[2]
        msg, status = self.dbcontrol.dbupdate(
            """update users set %s='%s' where username='%s';""" % (user_parament, user_value, user_username))
        result = msg
        return result

    def user_llists(self, *args):
        rows, status = self.dbcontrol.dbselect("""select * from users""")
        if status:
            pt = PrettyTable()
    ...
    def user_find(self, *args):
        user_username = args[0]
        rows, status = self.dbcontrol.dbselect("""select * from users where username='%s';""" % user_username)
        if status:
            pt = PrettyTable()
            pt.field_names = ["username", "age", "telephone", "email"]
            for i in rows:
    ...
    def user_display(self, *args):
        ...
        end_item = pagesize
        rows, status = self.dbcontrol.dbselect("""select * from users limit %s,%s;""" % (start_item, end_item))
        pt = PrettyTable()
        pt.field_names = ["username", "age", "telephone", "email"]
        for i in rows:
            pt.add_row([i[1], i[2], i[3], i[4]])
            ...
            while True:
                ...
                if check_result:
                    if user_list[0] == 'add':
                        ...
                        result = usermgmt.user_add(user_username, user_age, user_tel, user_email)
                        print(result)

                    elif user_list[0] == 'delete':
                        user_username = user_list[1]
                        result = usermgmt.user_delete(user_username, username)
                        print(result)
                        ...
                        result = usermgmt.user_update(user_username, user_parament, user_value)
                        print(result)

                    elif user_list[0] == 'list':
                        result = usermgmt.user_llists()
                        print(result)

                    elif user_list[0] == 'find':
                        result = usermgmt.user_find(user_list[1])
                        print(result)

                    elif user_list[0] == 'display':
                        result = usermgmt.user_display(user_list[1:])
                        print(result)

                    elif user_list[0] == 'export':
                        result = usermgmt.export_csv()
                        print(result)
