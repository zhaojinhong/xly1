...
# 自定义模块
from apps.deploy.forms import ApplyForm, DeployForm
from apps.deploy.models import Deploy
...
class ApplyListView(LoginRequiredMixin, PaginationMixin, ListView):
    ...
    def delete(self, request):
        ...
        print(delete_info)
        try:
            id = delete_info.get('apply_id', '')
            if id:
                deploy = self.model.objects.get(pk=id)
                if deploy.status == 0:
                    deploy.status = 3
                    deploy.save()
                    data = {
...
class DeployHistoryView(LoginRequiredMixin, PaginationMixin, ListView):
    ...
    model = Deploy
    template_name = "deploy/apply_history.html"
    context_object_name = "apply_list"
    paginate_by =10
    keyword = ''
