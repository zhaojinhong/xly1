...
class UserListView(LoginRequiredMixin,PermissionRequiredMixin,PaginationMixin,ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        #print(self.request.user.id,self.request.user.username,self.request.user.get_all_permissions())
...
class ModifyPasswordView(LoginRequiredMixin,View):
    ...
    def post(self, request, **kwargs):
        ...
        # data = QueryDict(request.body).dict()
        # print(data)  # {'id': '7', 'username': 'aa', 'name_cn': 'bb', 'phone': '13305779168'}
        # _userForm = ModifyPasswordFor(request.POST)
        # if _userForm.is_valid():
        #      try:
        #
        #
        ...
        #
        #          data['password'] = _userForm.cleaned_data['password']
        #          print(data)
        #          user = UserProfile.objects.filter(id=id)
        #
        #          user.update(**data)
        ...
        #      # 获取所有的表单错误
        #      print(_userForm.errors)
        #      print(reverse("users:modify_pwd"))
        #      res = {'code': 1, "next_url": reverse("users:modify_pwd"), 'errmsg': _userForm.errors}
        #      #return render(request,reverse("users:modify_pwd"),{'errmsg': _userForm.errors})
